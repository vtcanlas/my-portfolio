 ...
 * Page class
 */
class PoolpicsGrid extends Poolpics
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PoolpicsGrid";
    ...
    public $FormName = "fpoolpicsgrid";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "PoolpicsGrid";
    ...
    public function emptyRow()
    {
        ...
        if ($CurrentForm->hasValue("x_active") && $CurrentForm->hasValue("o_active") && $this->active->CurrentValue != $this->active->DefaultValue) {
            return false;
        }
        return true;
    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'pic_id' first before field var 'x_pic_id'
        $val = $CurrentForm->hasValue("pic_id") ? $CurrentForm->getValue("pic_id") : $CurrentForm->getValue("x_pic_id");
        if (!$this->pic_id->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
            $this->pic_id->setFormValue($val);
        }
    ...
    public function restoreFormValues()
    {
        ...
        if (!$this->isGridAdd() && !$this->isAdd()) {
            $this->pic_id->CurrentValue = $this->pic_id->FormValue;
        }
    ...
    protected function validateForm()
    {
        ...
        if ($this->img->Required) {
            if ($this->img->Upload->FileName == "" && !$this->img->Upload->KeepFile) {
                $this->img->addErrorMessage(str_replace("%s", $this->img->caption(), $this->img->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        if ($this->img->Visible && !$this->img->ReadOnly && !$this->img->Upload->KeepFile) {
            if ($this->img->Upload->Value === null) {
                $rsnew['img'] = null;
            } else {
                $rsnew['img'] = $this->img->Upload->Value;
            }
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->getCurrentMasterTable() == "pool") {
            $this->pool_id->CurrentValue = $this->pool_id->getSessionValue();
        }
        ...
        if ($this->pool_id->getSessionValue() != "") {
            $rsnew['pool_id'] = $this->pool_id->getSessionValue();
        }

        // Update current values
        $this->setCurrentValues($rsnew);

        // Check referential integrity for master table 'poolpics'
        $validMasterRecord = true;
        $detailKeys = [];
        $detailKeys["pool_id"] = $this->pool_id->getSessionValue();
        $masterTable = Container("tbl_resort_details");
        $masterFilter = $this->getMasterFilter($masterTable, $detailKeys);
        if (!EmptyValue($masterFilter)) {
            $rsmaster = $masterTable->loadRs($masterFilter)->fetch();
