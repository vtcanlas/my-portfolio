 ...
 * Page class
 */
class ResrateList extends Resrate
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ResrateList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "ResrateAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "ResrateDelete";
        $this->MultiUpdateUrl = "ResrateUpdate";
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->resrate_id); // resrate_id
            $this->updateSort($this->pool_id); // pool_id
            $this->updateSort($this->priceadult); // priceadult
            $this->updateSort($this->pricechild); // pricechild
            $this->updateSort($this->priceadultspecial); // priceadultspecial
            $this->updateSort($this->pricechildspecial); // pricechildspecial
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                $orderBy = "";
                $this->setSessionOrderBy($orderBy);
                $this->resrate_id->setSort("");
                $this->pool_id->setSort("");
                $this->priceadult->setSort("");
                $this->pricechild->setSort("");
                $this->priceadultspecial->setSort("");
                $this->pricechildspecial->setSort("");
            }
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fresratelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fresratelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->resrate_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("resrate_id", $this->createColumnOption("resrate_id"));
            $option->add("pool_id", $this->createColumnOption("pool_id"));
            $option->add("priceadult", $this->createColumnOption("priceadult"));
            $option->add("pricechild", $this->createColumnOption("pricechild"));
            $option->add("priceadultspecial", $this->createColumnOption("priceadultspecial"));
            $option->add("pricechildspecial", $this->createColumnOption("pricechildspecial"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fresratesrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fresratesrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fresratelist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
