...
class UsersEdit extends Users
{
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("idno") ?? Key(0) ?? Route(2)) !== null) {
                $this->idno->setQueryStringValue($keyValue);
                $this->idno->setOldValue($this->idno->QueryStringValue);
            } elseif (Post("idno") !== null) {
                $this->idno->setFormValue(Post("idno"));
                $this->idno->setOldValue($this->idno->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("idno") ?? Route("idno")) !== null) {
                    $this->idno->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->idno->CurrentValue = null;
                }
            }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'idno' first before field var 'x_idno'
        $val = $CurrentForm->hasValue("idno") ? $CurrentForm->getValue("idno") : $CurrentForm->getValue("x_idno");
        if (!$this->idno->IsDetailKey) {
            $this->idno->setFormValue($val);
        }
        ...
        // Check field name 'pword' first before field var 'x_pword'
        $val = $CurrentForm->hasValue("pword") ? $CurrentForm->getValue("pword") : $CurrentForm->getValue("x_pword");
        if (!$this->pword->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->pword->Visible = false; // Disable update for API request
            } else {
                $this->pword->setFormValue($val);
            }
        }
    ...
    {
        global $CurrentForm;
        $this->idno->CurrentValue = $this->idno->FormValue;
        ...
        $this->pword->CurrentValue = $this->pword->FormValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // pword
            $this->pword->setupEditAttributes();
            $this->pword->EditValue = $Language->phrase("PasswordMask"); // Show as masked password
            $this->pword->PlaceHolder = RemoveHtml($this->pword->caption());
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
        if ($this->idno->Required) {
            if (!$this->idno->IsDetailKey && EmptyValue($this->idno->FormValue)) {
                $this->idno->addErrorMessage(str_replace("%s", $this->idno->caption(), $this->idno->RequiredErrorMessage));
            }
        }
        ...
        if (!$this->uname->Raw && Config("REMOVE_XSS") && CheckUsername($this->uname->FormValue)) {
            $this->uname->addErrorMessage($Language->phrase("InvalidUsernameChars"));
        }
        if ($this->pword->Required) {
            if (!$this->pword->IsDetailKey && EmptyValue($this->pword->FormValue)) {
                $this->pword->addErrorMessage(str_replace("%s", $this->pword->caption(), $this->pword->RequiredErrorMessage));
            }
        }
        if (!$this->pword->Raw && Config("REMOVE_XSS") && CheckPassword($this->pword->FormValue)) {
            $this->pword->addErrorMessage($Language->phrase("InvalidPasswordChars"));
        }
    ...
    protected function editRow()
    {
        ...
        // uname
        $this->uname->setDbValueDef($rsnew, $this->uname->CurrentValue, null, $this->uname->ReadOnly);

        // pword
        if (!IsMaskedPassword($this->pword->CurrentValue)) {
            $this->pword->setDbValueDef($rsnew, $this->pword->CurrentValue, null, $this->pword->ReadOnly || Config("ENCRYPTED_PASSWORD") && $rsold['pword'] == $this->pword->CurrentValue);
        }
