 ...
 * Page class
 */
class ResrateView extends Resrate
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ResrateView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("resrate_id") ?? Route("resrate_id")) !== null) {
            $this->RecKey["resrate_id"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("resrate_id") ?? Route("resrate_id")) !== null) {
                ...
                $this->RecKey["resrate_id"] = $this->resrate_id->QueryStringValue;
            } elseif (Post("resrate_id") !== null) {
                $this->resrate_id->setFormValue(Post("resrate_id"));
                $this->RecKey["resrate_id"] = $this->resrate_id->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->resrate_id->setQueryStringValue($keyValue);
                $this->RecKey["resrate_id"] = $this->resrate_id->QueryStringValue;
            } elseif (!$loadCurrentRecord) {
                $returnUrl = "ResrateList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                        ...
                        if (!$res) { // Load record based on key
                            ...
                            $returnUrl = "ResrateList"; // No matching record, return to list
                        }
                    break;
            }
        } else {
            $returnUrl = "ResrateList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
