 ...
 * Page class
 */
class TblResortPicturesEdit extends TblResortPictures
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'tbl_resort_pictures';

    // Page object name
    public $PageObjName = "TblResortPicturesEdit";
    ...
    public function __construct()
    {
        ...
        // Table object (tbl_resort_pictures)
        if (!isset($GLOBALS["tbl_resort_pictures"]) || get_class($GLOBALS["tbl_resort_pictures"]) == PROJECT_NAMESPACE . "tbl_resort_pictures") {
            $GLOBALS["tbl_resort_pictures"] = &$this;
        }

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'tbl_resort_pictures');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $tbl = Container("tbl_resort_pictures");
                $doc = new $class($tbl);
                $doc->Text = @$content;
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if ($pageName != $this->getListUrl()) { // Not List page
                    ...
                    if ($pageName == "TblResortPicturesView") {
                        $row["view"] = "1";
                    }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("TblResortPicturesList"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "TblResortPicturesList") {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($curVal != "") {
                ...
                if ($this->pool_id->ViewValue === null) { // Lookup from database
                    $filterWrk = "`pool_id`" . SearchString("=", $curVal, DATATYPE_NUMBER, "qcresorts_db2");
            ...
            if ($this->pool_id->ViewValue !== null) { // Load from cache
                ...
                if ($curVal == "") {
                    ...
                    $filterWrk = "`pool_id`" . SearchString("=", $this->pool_id->CurrentValue, DATATYPE_NUMBER, "qcresorts_db2");
                }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("TblResortPicturesList"), "", $this->TableVar, true);
        $pageId = "edit";
