 ...
 * Page class
 */
class ResrateTime2Edit extends ResrateTime2
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ResrateTime2Edit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("time_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->time_id->setQueryStringValue($keyValue);
                $this->time_id->setOldValue($this->time_id->QueryStringValue);
            } elseif (Post("time_id") !== null) {
                $this->time_id->setFormValue(Post("time_id"));
                $this->time_id->setOldValue($this->time_id->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("time_id") ?? Route("time_id")) !== null) {
                    $this->time_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->time_id->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("ResrateTime2List"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "ResrateTime2List") {
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'time_id' first before field var 'x_time_id'
        $val = $CurrentForm->hasValue("time_id") ? $CurrentForm->getValue("time_id") : $CurrentForm->getValue("x_time_id");
        if (!$this->time_id->IsDetailKey) {
            $this->time_id->setFormValue($val);
        }
    ...
    {
        global $CurrentForm;
        $this->time_id->CurrentValue = $this->time_id->FormValue;
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
        if ($this->time_id->Required) {
            if (!$this->time_id->IsDetailKey && EmptyValue($this->time_id->FormValue)) {
                $this->time_id->addErrorMessage(str_replace("%s", $this->time_id->caption(), $this->time_id->RequiredErrorMessage));
            }
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("ResrateTime2List"), "", $this->TableVar, true);
        $pageId = "edit";
