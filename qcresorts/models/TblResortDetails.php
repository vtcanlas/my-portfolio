...
/**
 * Table class for tbl_resort_details
 */
class TblResortDetails extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        switch ($CurrentLanguage) {
            case "en-US":
                $this->poolcat->Lookup = new Lookup('poolcat', 'tbl_resort_details', false, '', ["","","",""], '', '', [], [], [], [], [], [], '', '', "");
                break;
            default:
                $this->poolcat->Lookup = new Lookup('poolcat', 'tbl_resort_details', false, '', ["","","",""], '', '', [], [], [], [], [], [], '', '', "");
                break;
    ...
    public function getDetailUrl()
    {
        ...
        if ($detailUrl == "") {
            $detailUrl = "TblResortDetailsList";
        }
        return $detailUrl;
    }
    ...
    public function getSqlWhere() // Where
    {
        ...
        $this->DefaultFilter = "uname = '".CurrentUserName()."'";
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        // Cascade Update detail table 'poolrates'
        $cascadeUpdate = false;
        $rscascade = [];
        if ($rsold && (isset($rs['pool_id']) && $rsold['pool_id'] != $rs['pool_id'])) { // Update detail field 'pool_id'
            $cascadeUpdate = true;
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("poolrates")->loadRs("`pool_id` = " . QuotedValue($rsold['pool_id'], DATATYPE_NUMBER, 'DB'))->fetchAllAssociative();
            foreach ($rswrk as $rsdtlold) {
                $rskey = [];
                $fldname = 'rateid';
                $rskey[$fldname] = $rsdtlold[$fldname];
                $rsdtlnew = array_merge($rsdtlold, $rscascade);
                // Call Row_Updating event
                $success = Container("poolrates")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("poolrates")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("poolrates")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }
    ...
    public function delete(&$rs, $where = "", $curfilter = false)
    {
        ...
        // Cascade delete detail table 'poolrates'
        $dtlrows = Container("poolrates")->loadRs("`pool_id` = " . QuotedValue($rs['pool_id'], DATATYPE_NUMBER, "DB"))->fetchAllAssociative();
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("poolrates")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("poolrates")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("poolrates")->rowDeleted($dtlrow);
            }
        }
        if ($success) {
            try {
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("TblResortDetailsList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "TblResortDetailsEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "TblResortDetailsAdd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "TblResortDetailsView";
            case Config("API_ADD_ACTION"):
                return "TblResortDetailsAdd";
            case Config("API_EDIT_ACTION"):
                return "TblResortDetailsEdit";
            case Config("API_DELETE_ACTION"):
                return "TblResortDetailsDelete";
            case Config("API_LIST_ACTION"):
                return "TblResortDetailsList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("TblResortDetailsView", $parm);
        } else {
            $url = $this->keyUrl("TblResortDetailsView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "TblResortDetailsAdd?" . $parm;
        } else {
            $url = "TblResortDetailsAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("TblResortDetailsEdit", $parm);
        } else {
            $url = $this->keyUrl("TblResortDetailsEdit", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TblResortDetailsList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("TblResortDetailsAdd", $parm);
        } else {
            $url = $this->keyUrl("TblResortDetailsAdd", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TblResortDetailsList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl()
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("TblResortDetailsDelete");
        }
    }
