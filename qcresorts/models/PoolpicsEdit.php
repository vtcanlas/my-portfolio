 ...
 * Page class
 */
class PoolpicsEdit extends Poolpics
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PoolpicsEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "PoolpicsEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("pic_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->pic_id->setQueryStringValue($keyValue);
                $this->pic_id->setOldValue($this->pic_id->QueryStringValue);
            } elseif (Post("pic_id") !== null) {
                $this->pic_id->setFormValue(Post("pic_id"));
                $this->pic_id->setOldValue($this->pic_id->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("pic_id") ?? Route("pic_id")) !== null) {
                    $this->pic_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->pic_id->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("PoolpicsList"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "PoolpicsList") {
                ...
                if ($this->editRow()) {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "PoolpicsList") {
                            ...
                            $returnUrl = "PoolpicsList"; // Return list page content
                        }
                    }
    ...
    public function renderRow()
    {
        ...
        // pic_id
        $this->pic_id->RowCssClass = "row";

        // img
        $this->img->RowCssClass = "row";
            ...
            if ($this->pool_id->getSessionValue() != "") {
                ...
                if ($this->pool_id->ViewValue !== null) { // Load from cache
                    ...
                    if ($curVal == "") {
                        ...
                        $filterWrk = SearchFilter("`pool_id`", "=", $this->pool_id->CurrentValue, DATATYPE_NUMBER, "qcresorts_db2");
                    }
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
        if ($this->pic_id->Required) {
            if (!$this->pic_id->IsDetailKey && EmptyValue($this->pic_id->FormValue)) {
                $this->pic_id->addErrorMessage(str_replace("%s", $this->pic_id->caption(), $this->pic_id->RequiredErrorMessage));
            }
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("PoolpicsList"), "", $this->TableVar, true);
        $pageId = "edit";
