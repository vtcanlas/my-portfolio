...
<body>
    ...
    <main class="page landing-page">
        <section id="hero" class="clean-block clean-hero" style="background-image:url(&quot;assets/img/header.jpg&quot;);color:rgba(5, 5, 5, 0.80);">
            <div class="text">
                <h2>QCResorts</h2>
                <p>Browse the various resorts in Quezon City.</p>
            </div>
        </section>
        <div class="container py-4 py-xl-5" id="resortcard">
            <div class="row gy-4 row-cols-1 row-cols-md-2 row-cols-xl-3">

            <?php
              $sql = "SELECT * FROM pool JOIN poolpics ON pool.pool_id=poolpics.pool_id WHERE active='1'";
              ...
              if ($rs>0){
                while ($row = mysqli_fetch_assoc($result)){
                  echo '
                    <div class="col">
                        <div>
                            <img style="width:1080px;height:280px;" class="img-thumbnail" src="data:image/jpeg;base64,'.base64_encode($row['img']).'"/>
                            <div class="py-4" style="text-align: left;">
                                <h4>'.$row['pool_name'].'</h4>
                                <p>Starts at P4000</p>
                                <p style="text-align: justify">' .$row['pool_description']. '</p>
                                <form method="post" action="resortpage.php">
                                  <button type="submit" name="resortid" value="'.$row['pool_id'].'" class="btn btn-outline-primary btn-lg">View Resort</button>
                                </form>
                            </div>
                        </div>
                    </div>
        ...
        <section class="clean-block clean-info dark" style="color: rgb(41,33,37);--bs-body-bg: var(--bs-red);background: rgb(255,255,255);padding: 8px;"></section>
    </main>
    ...
    <script>
        ScrollReveal().reveal('#hero',  {distance:'20px',duration:2000});  
        ScrollReveal().reveal('#resortcard',  {distance:'200px',duration:2000});  
