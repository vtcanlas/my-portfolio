...
?>
<main class="edit">
<form name="fresdetailsedit" id="fresdetailsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="on">
<script>
...
<?php if ($Page->res_id->Visible) { // res_id ?>
    <div id="r_res_id"<?= $Page->res_id->rowAttributes() ?>>
        <label id="elh_resdetails_res_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->res_id->caption() ?><?= $Page->res_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="hidden" data-table="resdetails" data-field="x_res_id" data-hidden="1" name="x_res_id" id="x_res_id" value="<?= HtmlEncode($Page->res_id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fresdetailsedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fresdetailsedit", "x_date", jQuery.extend(true, {"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<span id="el_resdetails_pool_id">
    <select
        ...
        data-select2-id="fresdetailsedit_x_pool_id"
...
<script>
loadjs.ready("fresdetailsedit", function() {
    var options = { name: "x_pool_id", selectId: "fresdetailsedit_x_pool_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fresdetailsedit.lists.pool_id?.lookupOptions.length) {
        options.data = { id: "x_pool_id", form: "fresdetailsedit" };
    } else {
        options.ajax = { id: "x_pool_id", form: "fresdetailsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_resdetails_rate_id">
    <select
        ...
        data-select2-id="fresdetailsedit_x_rate_id"
        data-table="resdetails"
...
<script>
loadjs.ready("fresdetailsedit", function() {
    var options = { name: "x_rate_id", selectId: "fresdetailsedit_x_rate_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fresdetailsedit.lists.rate_id?.lookupOptions.length) {
        options.data = { id: "x_rate_id", form: "fresdetailsedit" };
    } else {
        options.ajax = { id: "x_rate_id", form: "fresdetailsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fresdetailsedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fresdetailsedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
