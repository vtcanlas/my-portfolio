...
?>
<main class="edit">
<form name="fpooledit" id="fpooledit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="on">
<script>
...
<span id="el_pool_poolcat">
    <select
        ...
        data-select2-id="fpooledit_x_poolcat"
        data-table="pool"
...
<script>
loadjs.ready("fpooledit", function() {
    var options = { name: "x_poolcat", selectId: "fpooledit_x_poolcat" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fpooledit.lists.poolcat?.lookupOptions.length) {
        options.data = { id: "x_poolcat", form: "fpooledit" };
    } else {
        options.ajax = { id: "x_poolcat", form: "fpooledit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php } ?>
    ...
    <input type="hidden" data-table="pool" data-field="x_pool_id" data-hidden="1" name="x_pool_id" id="x_pool_id" value="<?= HtmlEncode($Page->pool_id->CurrentValue) ?>">
<?php
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fpooledit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fpooledit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
