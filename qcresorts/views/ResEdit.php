...
?>
<main class="edit">
<form name="fresedit" id="fresedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="on">
<script>
...
<?php if ($Page->Id->Visible) { // Id ?>
    <div id="r_Id"<?= $Page->Id->rowAttributes() ?>>
        <label id="elh_res_Id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->Id->caption() ?><?= $Page->Id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->Id->cellAttributes() ?>>
<span id="el_res_Id">
<span<?= $Page->Id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->Id->getDisplayValue($Page->Id->EditValue))) ?>"></span>
<input type="hidden" data-table="res" data-field="x_Id" data-hidden="1" name="x_Id" id="x_Id" value="<?= HtmlEncode($Page->Id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<?php if ($Page->Start->Visible) { // Start ?>
    <div id="r_Start"<?= $Page->Start->rowAttributes() ?>>
        <label id="elh_res_Start" for="x_Start" class="<?= $Page->LeftColumnClass ?>"><?= $Page->Start->caption() ?><?= $Page->Start->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->Start->cellAttributes() ?>>
<span id="el_res_Start">
<input type="<?= $Page->Start->getInputTextType() ?>" name="x_Start" id="x_Start" data-table="res" data-field="x_Start" value="<?= $Page->Start->EditValue ?>" placeholder="<?= HtmlEncode($Page->Start->getPlaceHolder()) ?>" data-format-pattern="<?= HtmlEncode($Page->Start->formatPattern()) ?>"<?= $Page->Start->editAttributes() ?> aria-describedby="x_Start_help">
...
<script>
loadjs.ready(["fresedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fresedit", "x_Start", jQuery.extend(true, {"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<?php if ($Page->End->Visible) { // End ?>
    <div id="r_End"<?= $Page->End->rowAttributes() ?>>
        <label id="elh_res_End" for="x_End" class="<?= $Page->LeftColumnClass ?>"><?= $Page->End->caption() ?><?= $Page->End->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->End->cellAttributes() ?>>
<span id="el_res_End">
<input type="<?= $Page->End->getInputTextType() ?>" name="x_End" id="x_End" data-table="res" data-field="x_End" value="<?= $Page->End->EditValue ?>" placeholder="<?= HtmlEncode($Page->End->getPlaceHolder()) ?>" data-format-pattern="<?= HtmlEncode($Page->End->formatPattern()) ?>"<?= $Page->End->editAttributes() ?> aria-describedby="x_End_help">
...
loadjs.ready(["fresedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fresedit", "x_End", jQuery.extend(true, {"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<?php if ($Page->AllDay->Visible) { // AllDay ?>
    <div id="r_AllDay"<?= $Page->AllDay->rowAttributes() ?>>
        <label id="elh_res_AllDay" class="<?= $Page->LeftColumnClass ?>"><?= $Page->AllDay->caption() ?><?= $Page->AllDay->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->AllDay->cellAttributes() ?>>
<span id="el_res_AllDay">
<div class="form-check d-inline-block">
    <input type="checkbox" class="form-check-input<?= $Page->AllDay->isInvalidClass() ?>" data-table="res" data-field="x_AllDay" data-boolean name="x_AllDay" id="x_AllDay" value="1"<?= ConvertToBool($Page->AllDay->CurrentValue) ? " checked" : "" ?><?= $Page->AllDay->editAttributes() ?> aria-describedby="x_AllDay_help">
...
<?php if ($Page->Description->Visible) { // Description ?>
    <div id="r_Description"<?= $Page->Description->rowAttributes() ?>>
        <label id="elh_res_Description" for="x_Description" class="<?= $Page->LeftColumnClass ?>"><?= $Page->Description->caption() ?><?= $Page->Description->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->Description->cellAttributes() ?>>
<span id="el_res_Description">
<textarea data-table="res" data-field="x_Description" name="x_Description" id="x_Description" cols="35" rows="4" placeholder="<?= HtmlEncode($Page->Description->getPlaceHolder()) ?>"<?= $Page->Description->editAttributes() ?> aria-describedby="x_Description_help"><?= $Page->Description->EditValue ?></textarea>
...
<?php if ($Page->Url->Visible) { // Url ?>
    <div id="r_Url"<?= $Page->Url->rowAttributes() ?>>
        <label id="elh_res_Url" for="x_Url" class="<?= $Page->LeftColumnClass ?>"><?= $Page->Url->caption() ?><?= $Page->Url->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->Url->cellAttributes() ?>>
<span id="el_res_Url">
<input type="<?= $Page->Url->getInputTextType() ?>" name="x_Url" id="x_Url" data-table="res" data-field="x_Url" value="<?= $Page->Url->EditValue ?>" size="30" maxlength="255" placeholder="<?= HtmlEncode($Page->Url->getPlaceHolder()) ?>" data-format-pattern="<?= HtmlEncode($Page->Url->formatPattern()) ?>"<?= $Page->Url->editAttributes() ?> aria-describedby="x_Url_help">
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fresedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fresedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
