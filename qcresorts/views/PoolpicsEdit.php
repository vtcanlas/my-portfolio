...
?>
<main class="edit">
<form name="fpoolpicsedit" id="fpoolpicsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="on">
<script>
...
<?php if ($Page->pic_id->Visible) { // pic_id ?>
    <div id="r_pic_id"<?= $Page->pic_id->rowAttributes() ?>>
        <label id="elh_poolpics_pic_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->pic_id->caption() ?><?= $Page->pic_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="hidden" data-table="poolpics" data-field="x_pic_id" data-hidden="1" name="x_pic_id" id="x_pic_id" value="<?= HtmlEncode($Page->pic_id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_poolpics_pool_id">
    <select
        ...
        data-select2-id="fpoolpicsedit_x_pool_id"
        data-table="poolpics"
...
<script>
loadjs.ready("fpoolpicsedit", function() {
    var options = { name: "x_pool_id", selectId: "fpoolpicsedit_x_pool_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fpoolpicsedit.lists.pool_id?.lookupOptions.length) {
        options.data = { id: "x_pool_id", form: "fpoolpicsedit" };
    } else {
        options.ajax = { id: "x_pool_id", form: "fpoolpicsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fpoolpicsedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fpoolpicsedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
