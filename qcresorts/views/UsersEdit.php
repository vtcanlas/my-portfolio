...
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        ...
        // Add fields
        .setFields([
            ["idno", [fields.idno.visible && fields.idno.required ? ew.Validators.required(fields.idno.caption) : null], fields.idno.isInvalid],
...
<?php if ($Page->idno->Visible) { // idno ?>
    <div id="r_idno"<?= $Page->idno->rowAttributes() ?>>
        <label id="elh_users_idno" class="<?= $Page->LeftColumnClass ?>"><?= $Page->idno->caption() ?><?= $Page->idno->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->idno->cellAttributes() ?>>
<span id="el_users_idno">
<span<?= $Page->idno->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->idno->getDisplayValue($Page->idno->EditValue))) ?>"></span>
<input type="hidden" data-table="users" data-field="x_idno" data-hidden="1" name="x_idno" id="x_idno" value="<?= HtmlEncode($Page->idno->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<div class="input-group">
    <input type="password" name="x_pword" id="x_pword" autocomplete="new-password" data-field="x_pword" value="<?= $Page->pword->EditValue ?>" size="30" maxlength="50" placeholder="<?= HtmlEncode($Page->pword->getPlaceHolder()) ?>"<?= $Page->pword->editAttributes() ?> aria-describedby="x_pword_help">
