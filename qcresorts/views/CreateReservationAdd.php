...
// Page object
$CreateReservationAdd = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { CreateReservation: currentTable } });
var currentForm, currentPageID;
var fCreateReservationadd;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fCreateReservationadd = new ew.Form("fCreateReservationadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = fCreateReservationadd;

    // Add fields
    var fields = currentTable.fields;
    fCreateReservationadd.addFields([
    ...
    // Form_CustomValidate
    fCreateReservationadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fCreateReservationadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fCreateReservationadd.lists.pool_id = <?= $Page->pool_id->toClientList($Page) ?>;
    loadjs.done("fCreateReservationadd");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fCreateReservationadd" id="fCreateReservationadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_CreateReservation_pool_id">
    <select
        ...
        data-select2-id="fCreateReservationadd_x_pool_id"
...
<script>
loadjs.ready("fCreateReservationadd", function() {
    var options = { name: "x_pool_id", selectId: "fCreateReservationadd_x_pool_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fCreateReservationadd.lists.pool_id.lookupOptions.length) {
        options.data = { id: "x_pool_id", form: "fCreateReservationadd" };
    } else {
        options.ajax = { id: "x_pool_id", form: "fCreateReservationadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready(["fCreateReservationadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fCreateReservationadd", "x_date", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
