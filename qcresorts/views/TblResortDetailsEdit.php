...
?>
<main class="edit">
<form name="ftbl_resort_detailsedit" id="ftbl_resort_detailsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="on">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { tbl_resort_details: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var ftbl_resort_detailsedit;
...
<?php if ($Page->pool_description->Visible) { // pool_description ?>
    <div id="r_pool_description"<?= $Page->pool_description->rowAttributes() ?>>
        <label id="elh_tbl_resort_details_pool_description" for="x_pool_description" class="<?= $Page->LeftColumnClass ?>"><?= $Page->pool_description->caption() ?><?= $Page->pool_description->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->pool_description->cellAttributes() ?>>
<span id="el_tbl_resort_details_pool_description">
<textarea data-table="tbl_resort_details" data-field="x_pool_description" name="x_pool_description" id="x_pool_description" cols="35" rows="4" placeholder="<?= HtmlEncode($Page->pool_description->getPlaceHolder()) ?>"<?= $Page->pool_description->editAttributes() ?> aria-describedby="x_pool_description_help"><?= $Page->pool_description->EditValue ?></textarea>
...
<?php if ($Page->poolcat->Visible) { // poolcat ?>
    <div id="r_poolcat"<?= $Page->poolcat->rowAttributes() ?>>
        <label id="elh_tbl_resort_details_poolcat" for="x_poolcat" class="<?= $Page->LeftColumnClass ?>"><?= $Page->poolcat->caption() ?><?= $Page->poolcat->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<span id="el_tbl_resort_details_poolcat">
    <select
        ...
        data-select2-id="ftbl_resort_detailsedit_x_poolcat"
        data-table="tbl_resort_details"
...
<script>
loadjs.ready("ftbl_resort_detailsedit", function() {
    var options = { name: "x_poolcat", selectId: "ftbl_resort_detailsedit_x_poolcat" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (ftbl_resort_detailsedit.lists.poolcat?.lookupOptions.length) {
        options.data = { id: "x_poolcat", form: "ftbl_resort_detailsedit" };
    } else {
        options.ajax = { id: "x_poolcat", form: "ftbl_resort_detailsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
    options = Object.assign({}, ew.selectOptions, options, ew.vars.tables.tbl_resort_details.fields.poolcat.selectOptions);
...
<?php if ($Page->address->Visible) { // address ?>
    <div id="r_address"<?= $Page->address->rowAttributes() ?>>
        <label id="elh_tbl_resort_details_address" for="x_address" class="<?= $Page->LeftColumnClass ?>"><?= $Page->address->caption() ?><?= $Page->address->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->address->cellAttributes() ?>>
<span id="el_tbl_resort_details_address">
<textarea data-table="tbl_resort_details" data-field="x_address" name="x_address" id="x_address" cols="35" rows="4" placeholder="<?= HtmlEncode($Page->address->getPlaceHolder()) ?>"<?= $Page->address->editAttributes() ?> aria-describedby="x_address_help"><?= $Page->address->EditValue ?></textarea>
...
<?php } ?>
    ...
    <input type="hidden" data-table="tbl_resort_details" data-field="x_pool_id" data-hidden="1" name="x_pool_id" id="x_pool_id" value="<?= HtmlEncode($Page->pool_id->CurrentValue) ?>">
<?php
    if (in_array("poolpics", explode(",", $Page->getCurrentDetailTable())) && $poolpics->DetailEdit) {
?>
...
<?php } ?>
<?php
    if (in_array("poolrates", explode(",", $Page->getCurrentDetailTable())) && $poolrates->DetailEdit) {
?>
<?php if ($Page->getCurrentDetailTable() != "") { ?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="ftbl_resort_detailsedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="ftbl_resort_detailsedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
...
loadjs.ready("head", function() {
    ew.addEventHandlers("tbl_resort_details");
});
</script>
<script>
