...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fresrateedit = new ew.Form("fresrateedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fresrateedit;

    // Add fields
    var fields = currentTable.fields;
    fresrateedit.addFields([
        ["resrate_id", [fields.resrate_id.visible && fields.resrate_id.required ? ew.Validators.required(fields.resrate_id.caption) : null], fields.resrate_id.isInvalid],
    ...
    // Form_CustomValidate
    fresrateedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fresrateedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fresrateedit.lists.pool_id = <?= $Page->pool_id->toClientList($Page) ?>;
    loadjs.done("fresrateedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fresrateedit" id="fresrateedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->resrate_id->Visible) { // resrate_id ?>
    <div id="r_resrate_id"<?= $Page->resrate_id->rowAttributes() ?>>
        <label id="elh_resrate_resrate_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->resrate_id->caption() ?><?= $Page->resrate_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->resrate_id->cellAttributes() ?>>
<span id="el_resrate_resrate_id">
<span<?= $Page->resrate_id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->resrate_id->getDisplayValue($Page->resrate_id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="resrate" data-field="x_resrate_id" data-hidden="1" name="x_resrate_id" id="x_resrate_id" value="<?= HtmlEncode($Page->resrate_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<span id="el_resrate_pool_id">
    <select
        ...
        data-select2-id="fresrateedit_x_pool_id"
        data-table="resrate"
...
<script>
loadjs.ready("fresrateedit", function() {
    var options = { name: "x_pool_id", selectId: "fresrateedit_x_pool_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fresrateedit.lists.pool_id.lookupOptions.length) {
        options.data = { id: "x_pool_id", form: "fresrateedit" };
    } else {
        options.ajax = { id: "x_pool_id", form: "fresrateedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
