...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fpool2edit = new ew.Form("fpool2edit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fpool2edit;

    // Add fields
    var fields = currentTable.fields;
    fpool2edit.addFields([
    ...
    // Form_CustomValidate
    fpool2edit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpool2edit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    loadjs.done("fpool2edit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fpool2edit" id="fpool2edit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->pool_id->Visible) { // pool_id ?>
    <div id="r_pool_id"<?= $Page->pool_id->rowAttributes() ?>>
        <label id="elh_pool2_pool_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->pool_id->caption() ?><?= $Page->pool_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->pool_id->cellAttributes() ?>>
<span id="el_pool2_pool_id">
<span<?= $Page->pool_id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->pool_id->getDisplayValue($Page->pool_id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="pool2" data-field="x_pool_id" data-hidden="1" name="x_pool_id" id="x_pool_id" value="<?= HtmlEncode($Page->pool_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
