...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fapprovededit = new ew.Form("fapprovededit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fapprovededit;

    // Add fields
    var fields = currentTable.fields;
    fapprovededit.addFields([
    ...
    // Form_CustomValidate
    fapprovededit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fapprovededit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    loadjs.done("fapprovededit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fapprovededit" id="fapprovededit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->res_id->Visible) { // res_id ?>
    <div id="r_res_id"<?= $Page->res_id->rowAttributes() ?>>
        <label id="elh_approved_res_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->res_id->caption() ?><?= $Page->res_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="hidden" data-table="approved" data-field="x_res_id" data-hidden="1" name="x_res_id" id="x_res_id" value="<?= HtmlEncode($Page->res_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fapprovededit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fapprovededit", "x_date", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
