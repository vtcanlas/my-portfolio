...
// Page object
$TblResortPicturesAdd = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { tbl_resort_pictures: currentTable } });
var currentForm, currentPageID;
var ftbl_resort_picturesadd;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    ftbl_resort_picturesadd = new ew.Form("ftbl_resort_picturesadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = ftbl_resort_picturesadd;

    // Add fields
    var fields = currentTable.fields;
    ftbl_resort_picturesadd.addFields([
    ...
    // Form_CustomValidate
    ftbl_resort_picturesadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    ftbl_resort_picturesadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    ftbl_resort_picturesadd.lists.active = <?= $Page->active->toClientList($Page) ?>;
    ftbl_resort_picturesadd.lists.pool_id = <?= $Page->pool_id->toClientList($Page) ?>;
    loadjs.done("ftbl_resort_picturesadd");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="ftbl_resort_picturesadd" id="ftbl_resort_picturesadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_tbl_resort_pictures_pool_id">
    <select
        ...
        data-select2-id="ftbl_resort_picturesadd_x_pool_id"
        data-table="tbl_resort_pictures"
...
<script>
loadjs.ready("ftbl_resort_picturesadd", function() {
    var options = { name: "x_pool_id", selectId: "ftbl_resort_picturesadd_x_pool_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (ftbl_resort_picturesadd.lists.pool_id.lookupOptions.length) {
        options.data = { id: "x_pool_id", form: "ftbl_resort_picturesadd" };
    } else {
        options.ajax = { id: "x_pool_id", form: "ftbl_resort_picturesadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
